...
COMFY_OUTPUT_DIR = pathlib.Path(
    os.environ.get("COMFYUI_OUTPUT_DIR", str(pathlib.Path.home() / "workspace" / "ComfyUI" / "output"))
)
...
VAE = "ae.safetensors"
BASE_NEG = (
    "lowres, blurry, deformed hands, bad anatomy, extra fingers, duplicate character, "
    "cropped face, unreadable text, watermark, logo, overexposed, underexposed, messy composition"
)
...
def latest_temp(prefix):
    matches = sorted(glob.glob(str(COMFY_OUTPUT_DIR / f"{prefix}*.png")))
    if not matches:
...
for idx, entry in enumerate(PROMPTS, start=1):
    ...
    if target.exists():
        ...
        continue
    negative = ", ".join(
        x for x in [BASE_NEG, entry.get("negative", "").replace("\n", " ").strip()] if x
    )
